 ...
 scrn.tabIndex = 1;
 //Использование клика/тапа для игры по полю
...
//Использование клавиш для игры
 scrn.onkeydown = function keyDown(k) {
 	if (k.keyCode == 32 || k.keyCode == 87 || k.keyCode == 38)   // Для игры можно нажимать Пробел, W или стрелочку вверх
 	{
 		switch (state.curr) {
            //Если игра в "меню", начать игру
	        case state.getReady :
	            state.curr = state.Play;
	            SFX.start.play();
	            break;
            //Если игра идет - взмахнуть
	        case state.Play :
	            bird.flap();
	            break;
            //Если игра закончилась, вернуться в "меню"
	        case state.gameOver :
	            state.curr = state.getReady;
	            bird.speed = 0;
	            bird.y = 100;
